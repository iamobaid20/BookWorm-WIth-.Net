  ...
  useEffect(() => {
    if (cookies.user) {
      if (selectedLanguage == null) {
        fetch(
          `http://localhost:${config.port}/api/product/getByTypeNotInShelf/${typeId}/${cookies.user}`
        )
          .then((res) => res.json())
          .then((data) => setProducts(data));
      } else {
        fetch(
          `http://localhost:${config.port}/api/product/getByLanguageAndTypeNotInShelf/${selectedLanguage}/${typeId}/${cookies.user}`
        )
          .then((res) => res.json())
          .then((data) => setProducts(data));
      }
      ...
      if (selectedLanguage == null) {
        fetch(`http://localhost:${config.port}/api/product/getByType/${typeId}`)
          .then((res) => res.json())
          .then((data) => setProducts(data));
      } else {
        fetch(
          `http://localhost:${config.port}/api/product/get/${typeId}/${selectedLanguage}`
        )
          .then((res) => res.json())
          .then((data) => setProducts(data));
      }
  ...
  return (
    <>
      ...
      >
        {products
          .filter((product) =>
            selectedGenre ? product.genreId === selectedGenre : product
          )
          .map((product) => {
                  ...
                  <div className="col p-4 d-flex flex-column position-static">
                    ...
                    >
                      ...
                      >
                        ...
                        <Dropdown
                          style={{
                            display: product.isRentable ? "inline" : "none",
                            marginLeft: "10px",
                          }}
                          onSelect={(key) =>
          ...
          {selectedProduct && (
            <Modal.Body>
              ...
              <Container>
                ...
                <Row>
                  <Col>
                    <strong>Publisher:</strong> {selectedProduct.publisher}
                  </Col>
                  <Col>
                    <strong>Genre:</strong> {selectedProduct.genreId}
                  </Col>
                </Row>
              </Container>
            </Modal.Body>
